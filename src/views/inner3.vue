    ...
    >
      <!-- <img
        ...
        v-if="
          ...
          ($store.state.isFirstLoginData.mobileBind == 0 &&
      ...
      <div class="closeWinBox">
        ...
        <div class="closeWinContent">
          <div @click="copy" class="closeWinCopy" @mouseenter="copy()">
            公众号：海象游戏
            <span ref="btn" data-clipboard-text="海象游戏">(点击复制)</span>
          </div>
...
import userInfoMask from "./view3/userInfoMask";
import loginBox from "./view3/loginBox";
import accountAndPas from "./view3/accountAndPas";
import passWordLogin from "./view3/passWordLogin";
import phoneLogin from "./view3/phoneLogin";
import customerService from "./view3/customerService";
import wechat from "./view3/wechat";
import email from "./view3/email";
import emailDetails from "./view3/emailDetails";
import reconiseInfo from "./view3/reconiseInfo";
import bindPhone from "./view3/bindPhone";
import coupon from "./view3/coupon";
import payType from "./view3/payType";
import register from "./view3/register";
import wallet from "./view3/wallet";
import account from "./view3/account";
import alertPwd from "./view3/alertPwd";
import antiAddiction from "./view3/antiAddiction";

import agreement from "./view3/agreement"; //用户协议
...
export default {
  ...
  data() {
    return {
      ...
      // logoIcon2: require("../assets/img/logoRed.png"), //海象红点logo
      // logoIcon: require("../assets/img/cmLogo.png"),//草莓logo
      // logoIcon2: require("../assets/img/cmLogo.png"),//草莓红点logo
      logoIcon: require("../assets/img/hmLogo.png"),//虹猫logo
      logoIcon2: require("../assets/img/hmLogoRed.png"),//虹猫红点logo
      // gzhIcon: require("../assets/img/haixiang.jpg"), //海象游戏公众号
      gzhIcon: require("../assets/img/hongmao.jpg"), //虹猫VIP社公众号
      // gzhIcon: require("../assets/img/cmyx.jpg"),//草莓游戏公众号
      ...
      serviceInfo: {},
      isUserFCM: 0, //用户是否实名过
      isLonginFCM: 0, //是否开启游戏迷系统
      isAdult: 0, //是否是未成年实名
      isPayFCM: 0, //是否开启支付防沉迷
      timmer: null,
      idfa: "",
      os: "",
      gameData: "",
  ...
  mounted() {
    ...
    this.agentId = localStorage.getItem("agentId")
      ...
      : "";
    this.os = localStorage.getItem("os") ? localStorage.getItem("os") : "";
    ...
    // 获取签名--加密
    this.sign = hex_md5(
      (this.userInfo.uid ? this.userInfo.uid : new Date().getTime()) +
        this.appid +
        this.award +
        new Date().getTime() +
    ...
    window.onmessage = function (e) {
      ...
      if (obj.type == "send_user") {
        ...
        window.localStorage.setItem("serviceInfo", JSON.stringify(serviceInfo));
        if(!obj.adid){
          obj.adid = _this.adid;
        ...
        var baseUrl = protocal + "//risedragon.hnllhy.cn/llpassport/api/member/getUserInfo";
    ...
    // 获取iframe中加载的游戏url
    //老链接内跳转指定新链接的方案  ----未完结
    // window.location.href="https://h5.e2z.cn/v2/jc/inner.html?appid="+this.appid+"&adid="+this.adid+"&agentId="+this.agentId;
    this.url =
  ...
  methods: {
    ...
    getSize() {
      ...
      } else if (clientWidth >= 1180 && clientWidth < 1480) {
        root.style.fontSize = 100 * (clientWidth / k / 2) + "px";
      } else if (clientWidth >= 1480) {
        root.style.fontSize = 100 * (clientWidth / k / 2.5) + "px";
      } else {
    ...
    getAddictionOpen() {
      ...
      if (!this.userInfo.uid || this.userInfo.fcm == "1") {
        return;
      }
      var data = {
        adid: this.adid,
      ...
      this.$http.post(fun.apiDomain_go + "member/checkFCMState", data).then(
        function (res) {
          if (res.body.code == 200) {
            var data = res.body.data;
            if (data.isLonginFCM && !data.isUserFCM) {
              this.$store.dispatch("IsRealNameTipFun", true);
            } else if (data.isLonginFCM && data.isUserFCM && !data.isAdult) {
              localStorage.removeItem("userInfo");
              this.$store.dispatch("IsLoginBoxFun", true);
            }
            this.isUserFCM = data.isUserFCM;
            this.isLonginFCM = data.isLonginFCM;
            this.isPayFCM = data.isPayFCM;
