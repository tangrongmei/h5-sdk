  ...
  <div class="reconiseInfo">
    ...
    <div class="reconiseInfoContent">
      ...
      <div class="reconiseInfoRow">
        <input placeholder="请输入真实姓名" onkeyup="value=value.replace(/[^\u4E00-\u9FA5]/g,'')" type="text" maxlength="10" name="truename" v-model="truename" />
      </div>
      <div class="reconiseInfoRow">
        <input type="text" name="idcard" v-model="idcard" placeholder="请填写真实有效的身份信息" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setFcmInfo() {
      ...
      //实名认证
     this.$http
        .post(fun.apiDomain_go+'member/setFcm',data).then(
        function (res) {
