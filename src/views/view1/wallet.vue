  ...
  <div class="walletBox">
    <div class="wallet">
      <div class="headerTite">
        ...
        <span>我的钱包</span>
        <div class="closeImg">
          <img :src="outIcon" @click="closeWin" />
        </div>
      </div>
    ...
    >
      <!-- 兑换窗口 -->
      <exchange
        v-if="$store.state.winType.isExchange"
        v-bind="$attrs"
        v-on="$listeners"
        v-on:exchaBalance="getExchaBalance(arguments)"
      />
      <!-- 充值窗口 -->
      <recharge
        v-if="$store.state.winType.isRecharge"
        v-on:rechargeBalance="getRechargeBalance(arguments)"
      />
    </div>
  </div>
</template>
